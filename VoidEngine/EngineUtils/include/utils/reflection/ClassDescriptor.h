...
namespace utils {
	...
	struct ClassDescriptor : public TypeDescriptor {
		...
		ClassDescriptor(const utils::Name& name, size_t size, std::shared_ptr<ClassDescriptorData> classData)
			: TypeDescriptor(name, size), m_Data(std::move(classData)) {
	...
	private:
		...
		std::shared_ptr<ClassDescriptorData> m_Data;
	};
